...
exprToString (Ctor e1 e2 e3)      = "(" ++ (exprToString e1) ++ "< 1 && " ++ (exprToString e1) ++ "> -1" ++ "?" ++
                                    (exprToString e1) ++ ":" ++ "(" ++ (exprToString e2) ++ "< 1 && " ++ (exprToString e2) ++ "> -1" ++ "?" ++
                                    (exprToString e2) ++ ":" ++ "(" ++ (exprToString e3) ++ "< 1 && " ++ (exprToString e3) ++ "> -1" ++ "?" ++
                                    (exprToString e3) ++ ":" ++ (exprToString e1) ++ "*" ++ (exprToString e2) ++ "*" ++ (exprToString e1) ++ ")" ++ ")" ++ ")"
exprToString (Mine e1)           = "(" ++ (exprToString e1) ++ "> 1 || " ++ (exprToString e1) ++ "< -1" ++ "?" ++
...
eval x y VarX = x
eval x y VarY = y
eval x y (Sine ex) = sin(pi*(eval x y ex))
eval x y (Cosine ex) = cos(pi*(eval x y ex))
eval x y (Average ex1 ex2) = (((eval x y ex1) + (eval x y ex2))/2)
eval x y (Times ex1 ex2) = (eval x y ex1) * (eval x y ex2)
eval x y (Thresh ex1 ex2 ex3 ex4) = if ((eval x y ex1)<(eval x y ex2)) then (eval x y ex3) else (eval x y ex4)
eval x y (Mine ex1)  = if ((eval x y ex1) > 1 || (eval x y ex1) < -1)  then  ((eval x y ex1) / (eval x y ex1))
                        else (eval x y ex1)
eval x y (Ctor ex1 ex2 ex3)
  | (eval x y ex1) <= 1 && -1 <= (eval x y ex1) = (eval x y ex1)
  | (eval x y ex2) <= 1 && -1 <= (eval x y ex2) = (eval x y ex2)
  | (eval x y ex3) <= 1 && -1 <= (eval x y ex3) = (eval x y ex3)
  | otherwise = (eval x y ex1) * (eval x y ex2) * (eval x y ex3)
...
build d
  | r == 0  = Sine (build q)
  | r == 1  = Cosine (build q)
  | r == 2  = Average (build q) (build q)
  | r == 3  = Times (build q) (build q)
  | r == 4  = Thresh (build q) (build q) (build q) (build q)
  | r == 5  = Ctor (build q) (build q) (build q)
  | otherwise = Mine (build q)
  where
    r = rand 7
    q = d-1
