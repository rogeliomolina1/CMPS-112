...
additivePersistence n
  | n < 10    = 0
  | otherwise = 1 + additivePersistence (sumList ((digitsOfInt (n `div` 10)) ++ ((n `mod` 10):[])))
...
digitalRoot n
  | n < 10    = n
  | otherwise = digitalRoot (sumList ((digitsOfInt (n `div` 10)) ++ ((n `mod` 10):[])))
